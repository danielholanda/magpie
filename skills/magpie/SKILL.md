...
**Config template (single kernel):** Use `kernel:` with `id`, `type`, `source_files`, `working_dir`, `testcase_command`, optional `compile_command`, `env`. See `Magpie/kernel_config.yaml.example` and `examples/ck_gemm_add.yaml`.
...
Example: `examples/ck_grouped_gemm_compare.yaml`.
...
Example configs: `examples/benchmarks/benchmark_vllm_dsr1.yaml`, `docs/benchmark.md`.
...
1. Emit YAML matching the structure in `Magpie/kernel_config.yaml.example`: `kernel:` with `id`, `type` (hip|cuda|pytorch), `source_files`, `working_dir`, `testcase_command`, and optionally `compile_command`, `env`.
