...
def compile_hip(
    ...
    so_file_path = os.path.join(working_dir, source_file_name + ".so") if with_so else None
    ...
    try:
        # Compile to binary
        out_cmd = [
            "hipcc", "-O2", "-std=c++17", f"--offload-arch={gpu_arch}",
            "-g", *hip_file_path, "-o", out_file_path
        ]
